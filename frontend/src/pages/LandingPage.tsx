...
// ════════════════════════════════════════════════════════════════
// CONTINUOUS STORY: RAMESH'S COMPLETE PROCUREMENT JOURNEY
// ════════════════════════════════════════════════════════════════
...
export default function LandingPage() {
  ...
  return (
    <div className="kq-landing-wrapper" style={{ background: '#020c08', color: '#ffffff', minHeight: '100vh', width: '100%' }}>
      {/* ════════════════════════════════════════════════════════════════
        ...
        >
          ...
          <nav className="kq-nav-pill">
            {navItems.map((item) => (
              <a
                key={item.id}
                href={`#${item.id}`}
                className={`kq-nav-link ${activeNav === item.id ? 'active' : ''}`}
          ...
          <button className="kq-sign-in-btn" onClick={handleSignIn}>
            {isAuthenticated && user
              ? (role === 'ADMIN' ? 'Admin Dashboard →' : role === 'OPERATOR' ? 'Operator Dashboard →' : 'Kisan Dashboard →')
              : 'Sign in'}
          </button>
        </motion.header>
        ...
        {showDemoModal && (
          ...
          >
            ...
            >
              ...
              <div style={{ width: '100%', borderRadius: '14px', overflow: 'hidden', background: '#000000', margin: '12px 0', boxShadow: '0 6px 20px rgba(0, 0, 0, 0.15)' }}>
                <video
                  src="https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/hf_20260328_083109_283f3553-e28f-428b-a723-d639c617eb2b.mp4"
                  controls
                  autoPlay
                  playsInline
                  style={{ width: '100%', maxHeight: '260px', display: 'block', objectFit: 'cover' }}
                />
              </div>
